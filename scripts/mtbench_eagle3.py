...
DEFAULT_QUESTIONS_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), ".cache", "mtbench_question.jsonl"
)
...
HEADER = (
    f"{'category':<14} | {'n':>3} | {'toks':>6} | {'t/s':>7} | "
    f"{'accept':>6} | {'avg_l':>5}"
)
...
def main() -> int:
    ...
    p.add_argument("--questions", default=DEFAULT_QUESTIONS_PATH, help="MT-Bench question.jsonl (auto-downloaded if missing)")
    p.add_argument("--per-category", type=int, default=None, help="Use first N questions from each category")
    p.add_argument("--max-questions", type=int, default=None, help="Cap total number of questions (after per-category filter)")
